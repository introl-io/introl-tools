...
public class ActCodeResultCellFactory(IActCodeHoursProcessor actCodeHoursProcessor) : IActCodeResultCellFactory
{
    public IEnumerable<CellToAdd> CreateEmployeeCells(ActCodeParsedSourceModel sourceModel, ref int row)
    {
        ...
        // cells.AddRange(CreateEmployeeCells(sourceModel.Employees.First().Value!, sourceModel.StartDate, sourceModel.EndDate, ref row));
    ...
    private IEnumerable<CellToAdd> CreateEmployeeCells(ActCodeEmployee employee, DateOnly startDate, DateOnly endDate, ref int row)
    {
    ...
    private IEnumerable<CellToAdd> CreateActCodeCells(string activityCode,
        Dictionary<DateOnly, (double regHours, double otHours)> actCodeHours, DateOnly startDate, DateOnly endDate, ref int row)
    {
        var cells = new List<CellToAdd>();
        var date = startDate;
        ...
        while (date <= endDate)
        {
            ...
            cells.Add(new CellToAdd
            {
                ...
                Value = $"=SUM({ExcelUtils.GetCellLocation(column, row)},{ExcelUtils.GetCellLocation(column, row + 1)})"
            });
            date = date.AddDays(1);
            column++;
        }
